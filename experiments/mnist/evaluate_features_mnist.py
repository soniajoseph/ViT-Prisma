...
@click.option(
    "-s", "--sae-name", help="Name of the SAE file, in form "
     "`99184a8a-local-mnist-clean-expansion-16-layer-3/n_images_480003.pt`"
)
def main(sae_name: str):

    sae_cfg = MNIST_SAE_CONFIG
    ...
    train_data, val_data, test_data = load_mnist(DATA_DIR / "mnist", augmentation=False, visualisation=False, with_index=True)
    _, val_data_visualisation, test_data_visualisation = load_mnist(DATA_DIR / "mnist", augmentation=False, visualisation=True, with_index=True)
