...
class VisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x, npatch_to_keep, mask):
        ...
        pos_embed = self.pos_embed.reshape(8, -1, D)
        h_new = H // 16
        w_new = W // 16
        pos_embed = interpolate_pos_embed_online(pos_embed, [14, 14], [h_new, w_new]).reshape(1, -1, D)
...
def interpolate_pos_embed_online(
    ...
    pos_embed = pos_embed.reshape(-1, orig_size[0], orig_size[1], embedding_size).permute(0, 3, 1, 2)
...
def vit_large_mae_pretraining(pretrained=True, out_layers=None):
    ...
    backbone = model.trunk
    backbone.masked_image_modeling = False
    backbone.decoder = None
    return backbone
