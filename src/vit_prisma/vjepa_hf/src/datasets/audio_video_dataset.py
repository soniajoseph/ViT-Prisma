...
from librosa.feature import melspectrogram as melspectrogram
...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        while not loaded_video:
            audio, buffer, clip_indices = self.loadvideo_decord(sample)  # [T H W 3]
            loaded_video = len(buffer) > 0 and len(audio) > 0
            if not loaded_video:
        ...
        audio = split_into_clips(torch.tensor(audio))
        return buffer, audio, label, clip_indices
    ...
    def loadvideo_decord(self, sample):
        ...
        try:
            vr = AVReader(fname, num_threads=-1, ctx=cpu(0), mono=True)
        except Exception as e:
            warnings.warn(f"warning {e} for sample {fname=}")
            return [], [], None
        ...
        if self.duration is not None:
            try:
                fps = vr._AVReader__video_reader.get_avg_fps()
        ...
        audio, buffer = vr.get_batch(all_indices)
        ...
        audio = [reduce(a, "c f t -> c f", "mean") for a in audio]
        return audio, buffer, clip_indices
