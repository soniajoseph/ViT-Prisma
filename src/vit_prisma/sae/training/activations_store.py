...
class ActivationsStore:
    """
    Class for streaming images and generating and storing activations
    while training SAEs
    """
    ...
    def __init__(
        ...
        if create_dataloader:
            self.storage_buffer_train = self.get_buffer(self.cfg.n_batches_in_buffer // 2, 'train')
            if self.dataset_val is not None:
                self.storage_buffer_val = self.get_buffer(self.cfg.n_batches_in_buffer // 2, 'val')
    ...
    def setup_dataloaders(self):
        self.train_dataloader = DataLoader(
            ...
            collate_fn=self.collate_fn_vision,
            drop_last=True
        )
        ...
        if self.dataset_val is not None:
            self.val_dataloader = DataLoader(
                ...
                num_workers=self.num_workers,
                batch_size=self.cfg.store_batch_size_prompts,
                collate_fn=self.collate_fn_vision_eval,
                drop_last=True
            )
            self.val_dataloader_iter = iter(self.val_dataloader)

    @staticmethod
    def collate_fn_vision(data):
        imgs = [d[0] for d in data]
        return torch.stack(imgs, dim=0)

    @staticmethod
    def collate_fn_vision_eval(data):
        imgs = [d[0] for d in data]
        return torch.stack(imgs, dim=0)

    def get_batch_images(self, train_or_val='train'):
        if train_or_val == 'train':
            dataloader_iter = self.train_dataloader_iter
            dataloader = self.train_dataloader
        else:
            dataloader_iter = self.val_dataloader_iter
    ...
    def get_activations(self, batch_images: torch.Tensor):
        layers = self.cfg.hook_layer if isinstance(self.cfg.hook_layer, list) else [self.cfg.hook_layer]
    ...
    def get_buffer(self, n_batches_in_buffer: int, train_or_val='train'):
        batch_size = self.cfg.store_batch_size_prompts
        d_in = self.cfg.d_in
        total_size = batch_size * n_batches_in_buffer 
        context_size = self.cfg.context_size
        num_layers = len(self.cfg.hook_layer) if isinstance(self.cfg.hook_layer, list) else 1

        # Convert dtype and device to the correct types
        dtype = getattr(torch, self.cfg.dtype) if isinstance(self.cfg.dtype, str) else self.cfg.dtype
        ...
        for refill_batch_idx_start in range(0, total_size, batch_size):
            refill_batch_images = self.get_batch_images(train_or_val)
            refill_activations = self.get_activations(refill_batch_images)

            new_buffer[
                refill_batch_idx_start : refill_batch_idx_start + batch_size, ...
            ] = refill_activations

        new_buffer = new_buffer.reshape(-1, num_layers, d_in) # Flatten batches and patches b/c patches don't communicate with each other anyway
        new_buffer = new_buffer[torch.randperm(new_buffer.shape[0])]

        return new_buffer

    def get_data_loader(self, train_or_val='train') -> Iterator[Any]:
        batch_size = self.cfg.train_batch_size_tokens

        if train_or_val == 'train':
            mixing_buffer = torch.cat(
                [self.get_buffer(self.cfg.n_batches_in_buffer // 2, 'train'), self.storage_buffer_train],
                dim=0,
            )
        else:
            mixing_buffer = torch.cat(
                [self.get_buffer(self.cfg.n_batches_in_buffer // 2, 'val'), self.storage_buffer_val],
                dim=0,
            )
        ...
        if train_or_val == 'train':
            self.storage_buffer_train = mixing_buffer[: mixing_buffer.shape[0] // 2]
        else:
            self.storage_buffer_val = mixing_buffer[: mixing_buffer.shape[0] // 2]
    ...
    def next_batch(self, train_or_val='train'):
        if train_or_val == 'train':
        ...
        try:
            ...
            if train_or_val == 'train':
                self.dataloader_train = self.get_data_loader('train')
                return next(self.dataloader_train)
            else:
                self.dataloader_val = self.get_data_loader('val')
                return next(self.dataloader_val)
...
# from vit_prisma.sae.sae import SAE

# # from sae_lens.tokenization_and_batching import concat_and_batch_sequences


# # TO DO: rewrite this for vision, there's no tokenization
...
#         self,
#         model: HookedRootModule,
#         dataset_train_path: HfDataset | str,
#         dataset_val_path: HfDataset | str,
#         streaming: bool,
#         hook_name: str,
#         hook_layer: int,
#         hook_head_index: int | None,
