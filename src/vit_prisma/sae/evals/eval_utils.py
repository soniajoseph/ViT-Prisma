...
from vit_prisma.utils.data_utils.cifar.cifar10_dict import cifar10_labels
...
def get_text_embeddings(model_name, original_text, source="clip", batch_size=32, local_model=None):  # TODO EdS:
    if source == "clip":
...
def visualize_sparsities(
    ...
    # TODO EdS: added
    log_freq_tokens = torch.clamp(log_freq_tokens, min=float('-100'))
    log_freq_images = torch.clamp(log_freq_images, min=float('-100'))
    hist(
        cfg,
        log_freq_tokens,
...
def image_patch_heatmap(activation_values, image_size=224, pixel_num=14):
    # print(f"Creating a heatmap of size {image_size} ({pixel_num} patches)")
...
def hist(cfg, tensor, save_name, show=False, to_wandb=False, renderer=None, **kwargs):
    ...
    save_figure_dir = Path(cfg.sae_path).parent / "sparsity_histograms"
    save_figure_dir.mkdir(parents=True, exist_ok=True)
    # save_figure_dir = os.path.join(cfg.sae_path, "sparsity_histograms")
    # Ensure the directory exists
    # os.makedirs(save_figure_dir, exist_ok=True)
    print(f"Saving sparsity histograms to: {save_figure_dir}")
    ...
    # write_image(histogram_fig, svg_path)  # TODO EdS:
