...
from vit_planetarium.dataloaders.circle import CircleDataset
...
class TestTrainingFunction(unittest.TestCase):
        ...
        def setUp(self):
            ...
            self.train_dataset, self.val_dataset = CircleDataset('train', transform=transform), CircleDataset('test', transform=transform)
    
            self.config = GlobalConfig()

            # Define a simple model
            self.model = timm.create_model('vit_base_patch16_224', pretrained=True)
            self.model.head = nn.Linear(self.model.head.in_features, 60)
