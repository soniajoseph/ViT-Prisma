...
def test_loading_open_clip():
    ...
    for name, param in og_model.named_parameters():
    ...
    print()
    print("original model output shape", og_image_embedding.shape)
    ...
    assert torch.allclose(hooked_output, og_image_embedding, atol=TOLERANCE), f"Model output diverges! Max diff: {torch.max(torch.abs(hooked_output - og_image_embedding))}"

    print(f"Test passed successfully!")
    print(f"Max difference between outputs: {max_diff}")
    print(f"Hooked model output shape: {hooked_output.shape}")
    print(f"Original model output shape: {og_image_embedding.shape}")
    print(f"Hooked model output (first few values): {hooked_output[0, :5]}")
    print(f"Original model output (first few values): {og_image_embedding[0, :5]}")
